...
Every time the Commander's employees pass each other in the hall, each of them must
stop and salute each other - one at a time - before resuming their path.
A salute is five seconds long, so each exchange of salutes takes a full
ten seconds (Commander Lambda's salute is a bit, er, involved). 
You think that by removing the salute requirement, you could save several 
collective hours of employee time per day. But first, you need to show her 
...
and '-', an empty space. Every employee walks at the same speed either
to right or to the left, according to their direction.
...
def solution(s):
    ...
    for character in s:
        ...
        # Count every right walking employee
        if character == ">":
            right_walking += 1

        # If found any left walking employee, calculate number of salutes they
        # will generate with already seen right walking employees
        elif character == "<":
            salutes += right_walking * 2
